...
@section('title', '경기 정보 수정')

@section('content')
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">경기 #{{ $match->id }} 정보 수정</h5>
    </div>
    <div class="card-body">
        <form action="{{ route('matches.update', $match->id) }}" method="POST" id="match-form">
            @csrf
            @method('PUT')
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="date" class="form-control @error('match_date') is-invalid @enderror" 
                           id="match_date" name="match_date" value="{{ old('match_date', $match->match_date->format('Y-m-d')) }}" required>
                    @error('match_date')
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-select @error('winner') is-invalid @enderror" 
                        ...
                        <option value="blue" {{ old('winner', $match->winner) == 'blue' ? 'selected' : '' }}>블루팀</option>
                        <option value="red" {{ old('winner', $match->winner) == 'red' ? 'selected' : '' }}>레드팀</option>
                    </select>
                    @error('winner')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                                            ...
                                            <select class="form-select @error('blue_player_'.$i) is-invalid @enderror" 
                                                ...
                                                @foreach($players as $player)
                                                    <option value="{{ $player->id }}" 
                                                        {{ (old('blue_player_'.$i) ?? ($bluePlayers[$i-1]->player_id ?? '')) == $player->id ? 'selected' : '' }}>
                                            ...
                                            <select class="form-select @error('blue_champion_'.$i) is-invalid @enderror" 
                                                ...
                                                @foreach($champions as $champion)
                                                    <option value="{{ $champion->id }}" 
                                                        {{ (old('blue_champion_'.$i) ?? ($bluePlayers[$i-1]->champion_id ?? '')) == $champion->id ? 'selected' : '' }}>
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group">
                                                ...
                                                <input type="number" class="form-control @error('blue_kills_'.$i) is-invalid @enderror" 
                                                       ...
                                                       value="{{ old('blue_kills_'.$i, $bluePlayers[$i-1]->kills ?? 0) }}" min="0" required>
                                                @error('blue_kills_'.$i)
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group">
                                                ...
                                                <input type="number" class="form-control @error('blue_deaths_'.$i) is-invalid @enderror" 
                                                       ...
                                                       value="{{ old('blue_deaths_'.$i, $bluePlayers[$i-1]->deaths ?? 0) }}" min="0" required>
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group">
                                                ...
                                                <input type="number" class="form-control @error('blue_assists_'.$i) is-invalid @enderror" 
                                                       ...
                                                       value="{{ old('blue_assists_'.$i, $bluePlayers[$i-1]->assists ?? 0) }}" min="0" required>
                                    ...
                                    <div class="row g-2">
                                        <div class="col-md-6">
                                            <select class="form-select @error('red_player_'.$i) is-invalid @enderror" 
                                                ...
                                                @foreach($players as $player)
                                                    <option value="{{ $player->id }}" 
                                                        {{ (old('red_player_'.$i) ?? ($redPlayers[$i-1]->player_id ?? '')) == $player->id ? 'selected' : '' }}>
                                                        {{ $player->player_name }} ({{ $player->rank }})
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-md-6">
                                            <select class="form-select @error('red_champion_'.$i) is-invalid @enderror" 
                                                ...
                                                @foreach($champions as $champion)
                                                    <option value="{{ $champion->id }}" 
                                                        {{ (old('red_champion_'.$i) ?? ($redPlayers[$i-1]->champion_id ?? '')) == $champion->id ? 'selected' : '' }}>
                                                        {{ $champion->champion_name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group">
                                                ...
                                                <input type="number" class="form-control @error('red_kills_'.$i) is-invalid @enderror" 
                                                       ...
                                                       value="{{ old('red_kills_'.$i, $redPlayers[$i-1]->kills ?? 0) }}" min="0" required>
                                                @error('red_kills_'.$i)
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group">
                                                ...
                                                <input type="number" class="form-control @error('red_deaths_'.$i) is-invalid @enderror" 
                                                       ...
                                                       value="{{ old('red_deaths_'.$i, $redPlayers[$i-1]->deaths ?? 0) }}" min="0" required>
                                                @error('red_deaths_'.$i)
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        ...
                                        <div class="col-md-4">
                                            <div class="input-group">
                                                ...
                                                <input type="number" class="form-control @error('red_assists_'.$i) is-invalid @enderror" 
                                                       ...
                                                       value="{{ old('red_assists_'.$i, $redPlayers[$i-1]->assists ?? 0) }}" min="0" required>
